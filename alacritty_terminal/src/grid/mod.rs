...
impl<T> Grid<T> {
    ...
    /// Iterate over visible cells expanding the viewport by the requested number of lines.
    #[inline]
    pub fn display_iter_with_padding(
        &self,
        extra_top: usize,
        extra_bottom: usize,
    ) -> GridIterator<'_, T> {
        ...
        let top_line = Line(-(self.display_offset() as i32));
        let mut first_line = top_line - extra_top;
        let mut last_line = top_line + self.screen_lines().saturating_sub(1);
        last_line += extra_bottom;

        let topmost_line = self.topmost_line();
        if first_line < topmost_line {
            first_line = topmost_line;
        }

        let bottommost_line = self.bottommost_line();
        if last_line > bottommost_line {
            last_line = bottommost_line;
        }
        ...
        let start = Point::new(first_line - 1, last_column);
        let end = Point::new(last_line, last_column);
