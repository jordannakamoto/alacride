...
use crate::config::debug::Debug as DebugConfig;
...
impl Display {
    ...
    pub fn draw<T: EventListener>(
        ...
        let display_offset_raw = terminal.grid().display_offset();
        // Use smooth scroll renderer's calculation for expanded viewport
        let extra_top_lines = if display_offset_raw > 0 {
            // When scrolled, expand viewport to include more history
            display_offset_raw.min(100)
        } else {
            0
        };
        ...
        let display_offset_virtual = content.virtual_display_offset();
        ...
        self.validate_hint_highlights(display_offset_actual);
        ...
        let vi_cursor_viewport_point = vi_cursor_point
            .and_then(|cursor| term::point_to_viewport(display_offset_actual, cursor));
        self.damage_tracker.damage_vi_cursor(vi_cursor_viewport_point);
        self.damage_tracker.damage_selection(selection_range, display_offset_actual);
        ...
        let pixel_offset_for_frame = pixel_offset;
                ...
                if has_highlighted_hint {
                    ...
                    if should_highlight(highlighted_hint) || should_highlight(vi_highlighted_hint) {
                        let visible_line = cell
                            .point
                            .line
                            .saturating_sub(extra_top_lines)
                            .min(size_info.screen_lines().saturating_sub(1));
                        damage_tracker
                            .frame()
        ...
        if let Some(vi_cursor_point) = vi_cursor_point {
            ...
            let obstructed_column = Some(vi_cursor_point)
                .filter(|point| point.line == -(display_offset_actual as i32))
        ...
        let ime_position = match search_state.regex() {
            ...
            None => {
                ...
                match vi_cursor_viewport_point {
                    None => term::point_to_viewport(display_offset_actual, cursor_point)
