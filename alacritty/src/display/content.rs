...
pub struct RenderableContent<'a> {
    ...
    size: &'a SizeInfo,
    display_offset_actual: usize,
    display_offset_virtual: usize,
}
...
impl<'a> RenderableContent<'a> {
    pub fn new<T: EventListener>(
        ...
        extra_top_lines: usize,
        extra_bottom_lines: usize,
    ) -> Self {
        ...
        let mut terminal_content = term.renderable_content();

        let display_offset_actual = terminal_content.display_offset;
        if extra_top_lines != 0 || extra_bottom_lines != 0 {
            let grid = term.grid();
            terminal_content.display_iter =
                grid.display_iter_with_padding(extra_top_lines, extra_bottom_lines);
            terminal_content.display_offset = display_offset_actual.saturating_add(extra_top_lines);
        }

        let display_offset_virtual = terminal_content.display_offset;
        ...
        let cursor_point = term::point_to_viewport(display_offset_virtual, cursor_point).unwrap();
        ...
        Self {
            ...
            display_offset_virtual,
        }
    }

    /// Viewport offset.
    pub fn display_offset(&self) -> usize {
        self.display_offset_actual
    }

    /// Virtual offset used when expanding the renderable viewport.
    pub fn virtual_display_offset(&self) -> usize {
        self.display_offset_virtual
    }
    ...
    pub fn cursor(mut self) -> RenderableCursor {
        ...
        if let Some(point) =
            term::point_to_viewport(self.display_offset_actual, self.terminal_content.cursor.point)
        {
            self.cursor.point = Point::new(point.line, point.column);
        }
...
impl RenderableCell {
    fn new(content: &mut RenderableContent<'_>, cell: Indexed<&Cell>) -> Self {
        ...
        let display_offset_actual = content.display_offset();
        let viewport_start = Point::new(Line(-(display_offset_actual as i32)), Column(0));
        ...
        let point = term::point_to_viewport(content.virtual_display_offset(), cell_point).unwrap();
