...
impl TextRenderBatch for Batch {
    ...
    fn add_item_with_offset(
        ...
        let cell_width = size_info.cell_width() as i32;
        let cell_height = size_info.cell_height() as i32;
        let column_base = cell.point.column.0 as i32;
        let line_base = cell.point.line as i32;

        let x_px = column_base * cell_width;
        let y_px = line_base * cell_height;
        ...
        vertex.y = to_i16(y_bottom);
        vertex.glyph_y = to_i16(glyph_y_bottom);
        ...
        vertex.x = to_i16(x_px_wide);
        vertex.y = to_i16(y_top);
        vertex.glyph_x = to_i16(glyph_x_px_wide);
        vertex.glyph_y = to_i16(glyph_y_top);
        vertex.u = glyph.uv_left + glyph.uv_width;
